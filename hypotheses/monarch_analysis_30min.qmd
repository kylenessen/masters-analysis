...
# Fit Bayesian GAM with brms - QUICK TEST VERSION
global_model <- brm(
  global_formula,
  family = negbinomial(),
  data = model_data,
  ...
  warmup = 500,  # Reduced for testing
  backend = "rstan",  # Use rstan as fallback if cmdstanr not available
  control = list(adapt_delta = 0.90, max_treedepth = 10),  # Less strict for testing
...
### Diagnostic Plots for Global Model (COMMENTED OUT FOR DEBUGGING)

```{r global-diagnostics, fig.width=8, fig.height=6}
cat("\nDiagnostic plots section commented out until model fits successfully\n")
...
# plot(global_model, ask = FALSE, pars = c("b_Intercept", "b_smeanywindyspeed_1", "b_smeanyteymp_1"))
...
### Model Performance Summary (COMMENTED OUT FOR DEBUGGING)

```{r global-summary}
cat("\n=== DEBUGGING RESULTS ===\n")
cat("Data preparation completed successfully\n")
cat("Number of observations in final dataset:", nrow(model_data), "\n")
cat("Number of view IDs:", length(unique(model_data$view_id)), "\n")
cat("Formula created successfully\n")

cat("\nNext step: Uncomment the brm() call in the test-global-model section\n")
cat("if the data above looks correct.\n")
...
# cat("Effective sample size (bulk):", round(min(neff_ratio(global_model)) * 4000, 0), "\n")
...
# cat("\nIf this test runs successfully, you can proceed with the full model comparison!\n")
```