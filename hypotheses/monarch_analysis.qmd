...
prepare_lag_data <- function(master_df, wind_df, lag_minutes) {
  ...
  # First add the expected datetime to df_t_lag
  df_t_lag <- df_t_lag %>%
  ...
  # Calculate interval predictors more efficiently
  ...
  calculate_metrics <- function(i) {
    ...
    temp_subset <- master_df %>%
      filter(
        view_id == row$view_id,
        datetime >= row$datetime_t_minus_1,
    ...
    # Time delta
    time_delta <- as.numeric(difftime(row$datetime, row$datetime_t_minus_1, units = "mins"))
    
    # Combine all metrics
    c(wind_metrics, temp_metrics, sun_metrics, time_delta_mins = time_delta)
  }
  
  # Apply the function to all rows
  metrics_list <- lapply(1:nrow(final_df), calculate_metrics)
  metrics_df <- do.call(rbind, lapply(metrics_list, as.data.frame))
  
  # Combine with original data
  final_df_with_predictors <- cbind(final_df, metrics_df)
  
  # Remove rows with too many NAs in predictors
  final_clean <- final_df_with_predictors %>%
...
for (i in seq_along(model_formulas_30m)) {
  ...
  tryCatch({
    ...
    gamm_model <- gamm(
      ...
      family = nb(),
      data = data_30m,
      random = list(view_id = ~1),
      correlation = corAR1(form = ~ 1 | view_id)
    )
    
    # Store model
    models_30m[[model_name]] <- gamm_model
    aic_values_30m[model_name] <- AIC(gamm_model$lme)
...
for (i in seq_along(model_formulas_60m)) {
  ...
  tryCatch({
    ...
    # Store model
    models_60m[[model_name]] <- gamm_model
    aic_values_60m[model_name] <- AIC(gamm_model$lme)
...
for (i in seq_along(model_formulas_120m)) {
  ...
  tryCatch({
    ...
    # Store model
    models_120m[[model_name]] <- gamm_model
    aic_values_120m[model_name] <- AIC(gamm_model$lme)
...
for (i in seq_along(model_formulas_240m)) {
  ...
  tryCatch({
    ...
    # Store model
    models_240m[[model_name]] <- gamm_model
    aic_values_240m[model_name] <- AIC(gamm_model$lme)
...
for (i in seq_along(model_formulas_full_day)) {
  ...
  tryCatch({
    ...
    # Store model
    models_full_day[[model_name]] <- gamm_model
    aic_values_full_day[model_name] <- AIC(gamm_model$lme)
