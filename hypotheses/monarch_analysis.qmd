...
```{r models-30min, warning=FALSE, fig.width=8, fig.height=6}
# Define comprehensive Bayesian model formulas for brms
model_formulas_30m <- list(
  # 1. Baseline Model
  "1. Baseline" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                       (1 | view_id) + 
                                       ar(time = datetime, gr = view_id, p = 1)),
  
  # 2. Single Environmental Factor Models  
  "2a. Wind only" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                          s(mean_wind_speed) + 
                                          (1 | view_id) + 
                                          ar(time = datetime, gr = view_id, p = 1)),
  "2b. Temperature only" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                                 s(mean_temp) + 
                                                 (1 | view_id) + 
                                                 ar(time = datetime, gr = view_id, p = 1)),
  "2c. Sunlight only" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                              s(sunlight_proportion) + 
                                              (1 | view_id) + 
                                              ar(time = datetime, gr = view_id, p = 1)),
  
  # 3. Additive Models (No Interactions)
  "3a. Wind + Temp" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                            s(mean_wind_speed) + 
                                            s(mean_temp) + 
                                            (1 | view_id) + 
                                            ar(time = datetime, gr = view_id, p = 1)),
  "3b. Wind + Sunlight" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                                s(mean_wind_speed) + 
                                                s(sunlight_proportion) + 
                                                (1 | view_id) + 
                                                ar(time = datetime, gr = view_id, p = 1)),
  "3c. Temp + Sunlight" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                                s(mean_temp) + 
                                                s(sunlight_proportion) + 
                                                (1 | view_id) + 
                                                ar(time = datetime, gr = view_id, p = 1)),
  "3d. All additive" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                             s(mean_wind_speed) + 
                                             s(mean_temp) + 
                                             s(sunlight_proportion) + 
                                             (1 | view_id) + 
  ...
  # 4. Single Interaction Models
  "4a. Wind x Temp interaction" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                                        s(mean_wind_speed) + 
                                                        s(mean_temp) + 
                                                        s(sunlight_proportion) + 
                                                        ...
                                                        (1 | view_id) + 
                                                        ar(time = datetime, gr = view_id, p = 1)),
  "4b. Wind x Sunlight interaction" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                                            s(mean_wind_speed) + 
                                                            s(mean_temp) + 
                                                            s(sunlight_proportion) + 
                                                            ...
                                                            (1 | view_id) + 
                                                            ar(time = datetime, gr = view_id, p = 1)),
  "4c. Temp x Sunlight interaction" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                                            s(mean_wind_speed) + 
                                                            s(mean_temp) + 
                                                            s(sunlight_proportion) + 
  ...
  # 5. Multiple Interaction Models
  "5a. Wind x Temp + Wind x Sun" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                                         s(mean_wind_speed) + 
                                                         s(mean_temp) + 
                                                         s(sunlight_proportion) + 
                                                         t2(mean_wind_speed, mean_temp) + 
                                                         ...
                                                         (1 | view_id) + 
                                                         ar(time = datetime, gr = view_id, p = 1)),
  "5b. Wind x Temp + Temp x Sun" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                                         s(mean_wind_speed) + 
                                                         s(mean_temp) + 
                                                         s(sunlight_proportion) + 
                                                         t2(mean_wind_speed, mean_temp) + 
                                                         ...
                                                         (1 | view_id) + 
                                                         ar(time = datetime, gr = view_id, p = 1)),
  "5c. Wind x Sun + Temp x Sun" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                                        s(mean_wind_speed) + 
                                                        s(mean_temp) + 
                                                        s(sunlight_proportion) + 
                                                        t2(mean_wind_speed, sunlight_proportion) + 
  ...
  # 6. Global Model with All Interactions
  "6. Global model" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                            s(mean_wind_speed) + 
                                            s(mean_temp) + 
                                            s(sunlight_proportion) + 
                                            t2(mean_wind_speed, mean_temp) + 
...
# Check data before fitting
  ...
  # Store model
  models_30m[[model_name]] <- brm_model
  
  # Calculate LOO for model comparison (better than AIC for Bayesian models)
  loo_result <- loo(brm_model)
  loo_values_30m[model_name] <- loo_result$estimates["looic", "Estimate"]
  ...
  qqnorm(residuals_vals, main = paste("Q-Q Plot:", model_name))
...
# Model comparison table using LOO-CV
comparison_30m <- data.frame(
  Model = names(loo_values_30m),
  LOO_IC = round(loo_values_30m, 2)
) %>%
  arrange(LOO_IC)  # Lower LOO-IC indicates better model

cat("\n### Model Comparison (30-minute lag)\n")
cat("Using LOO-CV Information Criterion (lower is better)\n")
...
```{r models-60min, warning=FALSE, fig.width=8, fig.height=6}
# Define comprehensive Bayesian model formulas for brms
model_formulas_60m <- list(
  # 1. Baseline Model
  "1. Baseline" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                       (1 | view_id) + 
                                       ar(time = datetime, gr = view_id, p = 1)),
...
for (i in seq_along(model_formulas_60m)) {
  ...
  tryCatch({
    ...
    # Store model
    models_60m[[model_name]] <- brm_model
    
    # Calculate LOO for model comparison (better than AIC for Bayesian models)
    loo_result <- loo(brm_model)
    loo_values_60m[model_name] <- loo_result$estimates["looic", "Estimate"]
...
comparison_60m <- data.frame(
  Model = names(loo_values_60m),
  LOO_IC = round(loo_values_60m, 2)
) %>%
  arrange(LOO_IC)  # Lower LOO-IC indicates better model
...
```{r models-120min, warning=FALSE, fig.width=8, fig.height=6}
# Define comprehensive Bayesian model formulas for brms
model_formulas_120m <- list(
  # 1. Baseline Model
  "1. Baseline" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                       (1 | view_id) + 
                                       ar(time = datetime, gr = view_id, p = 1)),
...
for (i in seq_along(model_formulas_120m)) {
  ...
  tryCatch({
    ...
    # Store model
    models_120m[[model_name]] <- brm_model
    
    # Calculate LOO for model comparison (better than AIC for Bayesian models)
    loo_result <- loo(brm_model)
    loo_values_120m[model_name] <- loo_result$estimates["looic", "Estimate"]
...
comparison_120m <- data.frame(
  Model = names(loo_values_120m),
  LOO_IC = round(loo_values_120m, 2)
) %>%
  arrange(LOO_IC)  # Lower LOO-IC indicates better model
...
```{r models-240min, warning=FALSE, fig.width=8, fig.height=6}
# Define comprehensive Bayesian model formulas for brms
model_formulas_240m <- list(
  # 1. Baseline Model
  "1. Baseline" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                       (1 | view_id) + 
                                       ar(time = datetime, gr = view_id, p = 1)),
...
for (i in seq_along(model_formulas_240m)) {
  ...
  tryCatch({
    ...
    # Store model
    models_240m[[model_name]] <- brm_model
    
    # Calculate LOO for model comparison (better than AIC for Bayesian models)
    loo_result <- loo(brm_model)
    loo_values_240m[model_name] <- loo_result$estimates["looic", "Estimate"]
...
comparison_240m <- data.frame(
  Model = names(loo_values_240m),
  LOO_IC = round(loo_values_240m, 2)
) %>%
  arrange(LOO_IC)  # Lower LOO-IC indicates better model
...
```{r models-full-day, warning=FALSE, fig.width=8, fig.height=6}
# Define comprehensive Bayesian model formulas for brms
model_formulas_full_day <- list(
  # 1. Baseline Model
  "1. Baseline" = bf(total_butterflies ~ s(abundance_t_minus_1, k = 20) + 
                                       (1 | view_id) + 
                                       ar(time = datetime, gr = view_id, p = 1)),
...
for (i in seq_along(model_formulas_full_day)) {
  ...
  tryCatch({
    ...
    # Store model
    models_full_day[[model_name]] <- brm_model
    
    # Calculate LOO for model comparison (better than AIC for Bayesian models)
    loo_result <- loo(brm_model)
    loo_values_full_day[model_name] <- loo_result$estimates["looic", "Estimate"]
    ...
    cat("Error fitting model:", e$message, "\n")
    cat("This might be due to insufficient data or convergence issues\n")
    loo_values_full_day[model_name] <- NA
  })
}
```
...
comparison_full_day <- data.frame(
  Model = names(loo_values_full_day),
  LOO_IC = round(loo_values_full_day, 2)
) %>%
  arrange(LOO_IC)  # Lower LOO-IC indicates better model
...
# Combine all LOO-CV comparisons
all_comparisons <- bind_rows(
  ...
  select(Lag, Model, LOO_IC) %>%
  pivot_wider(names_from = Lag, values_from = LOO_IC)

cat("LOO-CV Information Criterion Values Across All Time Lags:\n")
...
best_models <- data.frame(
  ...
  Best_LOO_IC = c(
    comparison_30m$LOO_IC[1],
    comparison_60m$LOO_IC[1],
    comparison_120m$LOO_IC[1],
    comparison_240m$LOO_IC[1],
    comparison_full_day$LOO_IC[1]
  )
)

cat("\n\nBest Model for Each Time Lag (Bayesian Analysis):\n")
cat("Based on LOO-CV Information Criterion (lower is better)\n")
