...
format:
  pdf: 
    include-in-header: |
      ...
      \usepackage{wrapfig}
      \usepackage{caption}
      % optional: avoid ulem’s underlining side effects
...
# Simplified table options for PDF compatibility
...
p1 <- ggplot(df, aes(x = abundance_index_t)) +
    ...
    labs(
        x = "Butterfly Abundance at Time t", y = "Count",
...
p2 <- ggplot(df, aes(x = sustained_minutes_above_2ms)) +
    ...
    scale_y_continuous(expand = c(0, 0)) +
    labs(
        x = "Minutes of Sustained Wind > 2 m/s", y = "Count",
...
p3 <- ggplot(df, aes(x = abundance_index_t_minus_1, y = abundance_index_t)) +
    ...
    labs(
        x = "Abundance at t-1", y = "Abundance at t",
...
p4 <- ggplot(df, aes(x = factor(sustained_minutes_above_2ms > 0), y = prop_change)) +
    ...
    labs(
        x = "Wind Exposure (Any minutes > 2 m/s)", y = "Proportional Change",
...
plotResiduals(sim_res,
    form = df_complete$sustained_minutes_above_2ms,
    main = "Residuals vs. Wind Minutes"
)
plotResiduals(sim_res,
    form = df_complete$ambient_temp,
...
wind_pred <- ggpredict(m1_threshold,
    ...
    condition = c(
        log_lag_abundance = mean(df_complete$log_lag_abundance),
        temp_std = 0,
        sun_std = 0
    )
)
...
p_wind <- ggplot(wind_pred, aes(x = x, y = predicted)) +
    ...
    geom_hline(
        yintercept = mean(df_complete$abundance_index_t),
        linetype = "dashed", alpha = 0.5
    ) +
    labs(
        x = "Minutes of Sustained Wind > 2 m/s",
...
model_comparison_robust <- data.frame(
    ...
    df = c(
        df.residual(m1_threshold), df.residual(m_interaction),
