...
# Set up for sequential execution with GPU acceleration
...
null_priors <- c(
    ...
    prior(inv_gamma(2, 1), class = shape), # Prevents zero, weakly informative
    prior(exponential(0.1), class = sd), # Weakly informative, allows large variance
    prior(beta(2, 2), class = ar) # Weakly informative for AR, slight preference for 0.5
)
...
smooth_priors <- c(
    ...
    prior(inv_gamma(2, 1), class = shape), # Prevents zero issue
    ...
    prior(beta(2, 2), class = ar) # Symmetric around 0.5
)
...
```{r fit-function}
# Sequential model fitting function with GPU support
    ...
    tryCatch(
        {
            ...
            # Add OpenCL GPU acceleration if available
            if (opencl_available) {
                brm_args$opencl <- brms::opencl(ids = c(0L, 0L))
                brm_args$threads <- brms::threading(threads = 2)
                cat("Using GPU acceleration\n")
            } else {
                cat("Using CPU only\n")
            }
