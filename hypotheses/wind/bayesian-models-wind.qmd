...
dir.create(here("hypotheses", "wind", "models"), showWarnings = FALSE, recursive = TRUE)

# Sequential model fitting function with caching (CPU only)
fit_single_model <- function(formula, prior, model_name = "model", force_refit = FALSE) {
    # Create model file path
    model_file <- here("hypotheses", "wind", "models", paste0(model_name, ".rds"))
    
    # Check if model already exists and we're not forcing a refit
    if (file.exists(model_file) && !force_refit) {
        cat("\nLoading cached model:", model_name, "\n")
    ...
    # If we reach here, we need to fit a new model
    cat("\nFitting new model:", model_name, "(CPU only)\n")
