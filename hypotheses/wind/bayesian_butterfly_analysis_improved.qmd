...
# Add OpenCL if available
if (opencl_available) {
    test_args$opencl <- opencl(ids = c(0, 0))
    cat("Testing with GPU acceleration...\n")
} else {
    cat("Testing with CPU only...\n")
}

# Test with just null models first
test_null_wind <- do.call(brm, test_args)
...
```{r loo-improved}
# LOO with fallback options for existing models
cat("\n=== MODEL COMPARISON WITH LOO ===\n")

# Function to compute LOO with fallback to standard if moment matching fails
compute_loo_safe <- function(model, model_name) {
    cat("Computing LOO for", model_name, "...")
    
    loo_result <- tryCatch({
        # Try with moment matching first (requires save_pars)
        loo(model, moment_match = TRUE)
    }, error = function(e) {
        cat(" moment matching failed, using standard LOO...")
        # Fallback to standard LOO
        tryCatch({
            loo(model)
        }, error = function(e2) {
...
# Filter out NULL results before comparison
wind_loo_valid <- wind_loo[!sapply(wind_loo, is.null)]
threshold_loo_valid <- threshold_loo[!sapply(threshold_loo, is.null)]

# Comparisons only if we have at least 2 valid models
if (length(wind_loo_valid) >= 2) {
    wind_comparison <- loo_compare(wind_loo_valid)
    print("=== WIND MODELS ===")
    print(wind_comparison)
    
    # Model weights
    wind_weights <- loo_model_weights(wind_loo_valid)
} else {
    cat("Not enough valid LOO results for wind model comparison\n")
    wind_comparison <- NULL
    wind_weights <- NULL
}

if (length(threshold_loo_valid) >= 2) {
    threshold_comparison <- loo_compare(threshold_loo_valid)
    print("\n=== THRESHOLD MODELS ===")
    print(threshold_comparison)
    
    # Model weights  
    threshold_weights <- loo_model_weights(threshold_loo_valid)
} else {
    cat("Not enough valid LOO results for threshold model comparison\n")
