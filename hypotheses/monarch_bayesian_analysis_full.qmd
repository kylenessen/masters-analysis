...
# Since threading is disabled with AR models, maximize parallel chains
CHAINS <- 4 # standard for diagnostics
PCHAINS <- 4 # run all 4 chains in parallel (uses 4 cores)
# THREADS disabled due to AR terms
ITER <- 4000 # full sampling
WARMUP <- 1000
SEED <- 20250812
...
cat(
    ...
    "| cores used =", PCHAINS, "(threading disabled with AR models)\n"
)
...
brms_common <- list(
    ...
    warmup          = WARMUP,
    chains          = CHAINS,
    cores           = PCHAINS,  # Use cores instead of parallel_chains for cmdstanr
