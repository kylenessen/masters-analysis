...
prepare_lag_data <- function(master_df, wind_df, lag_minutes) {
    ...
    # Create lagged copy, shifted forward by lag to align t-Δ with t
    df_t_lag <- df_t %>%
        rename(
    ...
    # Join exact matches on (view_id, datetime == datetime_expected)
    final_df <- df_t %>%
        left_join(
            df_t_lag,
    ...
    wind_ag <- wind_dt[
        ...
        .(
            ...
            minutes_above_2mps = sum(speed > 2, na.rm = TRUE) # assumes ~1-min cadence
        ),
        by = .EACHI
    ]
