...
## Time Series Plots by Deployment

```{r}
# Create time series plots of butterfly counts for each deployment
library(scales)

# Function to create time series plots by deployment
create_time_series_plots <- function(data, dataset_name) {
    cat(paste0("\n=== TIME SERIES PLOTS FOR: ", dataset_name, " ===\n"))
    ...
    # Get deployment info for better labels
    deployment_info <- data %>%
        select(view_id, wind_meter_name) %>%
        distinct() %>%
        arrange(view_id)
    
    cat("Deployments in dataset:\n")
    print(deployment_info)
    
    # Create individual time series plot for each deployment
    plots <- list()
    
    for (i in 1:nrow(deployment_info)) {
        vid <- deployment_info$view_id[i]
        wm_name <- deployment_info$wind_meter_name[i]
        ...
        if (nrow(deployment_data) > 0) {
            p <- ggplot(deployment_data, aes(x = datetime, y = total_butterflies)) +
                geom_point(alpha = 0.6, size = 1) +
                geom_line(alpha = 0.4) +
                scale_x_datetime(labels = date_format("%m/%d %H:%M"), 
                               breaks = date_breaks("1 day")) +
                labs(
                    title = paste0("Deployment ", vid, " (", wm_name, ")"),
                    subtitle = paste0("n = ", nrow(deployment_data), " observations"),
                    x = "Date/Time",
                    y = "Total Butterflies"
                ) +
                theme_minimal() +
            ...
            print(p)
            plots[[paste0("deployment_", vid)]] <- p
        }
    }
    
    # Create a combined plot showing all deployments
    if (length(unique(data$view_id)) <= 8) {  # Only if not too many deployments
        combined_plot <- ggplot(data, aes(x = datetime, y = total_butterflies, color = factor(view_id))) +
            geom_point(alpha = 0.6, size = 0.5) +
            geom_line(alpha = 0.4) +
            scale_x_datetime(labels = date_format("%m/%d"), 
                           breaks = date_breaks("1 week")) +
            labs(
                title = paste0("All Deployments - ", dataset_name),
                x = "Date",
                y = "Total Butterflies",
                color = "View ID"
            ) +
            ...
            facet_wrap(~ view_id, scales = "free", ncol = 2)
...
# Create time series plots for the 30-minute lag data (most comprehensive)
if (nrow(data_30m) > 0) {
    ts_plots_30m <- create_time_series_plots(data_30m, "30-Minute Lag")
}
```
