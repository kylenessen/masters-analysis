...
There were no butterflies for UDMH1, so it is a moot point. Woohoo!

## Combined Export

```{r}
#| label: combine-and-export-all-dbs
# Read all Wind tables, tag with wind_meter_name, and export CSV
...
wind_all <- db_files %>%
  mutate(data = purrr::map(db_path, fetch_all_wind)) %>%
  tidyr::unnest(data) %>%
  relocate(wind_meter_name, .before = time)

# Preview combined data
wind_all %>% arrange(wind_meter_name, time) %>% head(10)

# Export to CSV in data folder
out_path <- file.path("data", "wind_all.csv")
readr::write_csv(wind_all, out_path)
glue("Wrote {scales::comma(nrow(wind_all))} rows to {out_path}")
```
