...
# Smooth baseline
fit_model_safe(
    "butterfly_diff_sqrt ~ s(max_butterflies_t_1, k = k_baseline, bs = \"ts\") + s(lag_duration_hours, k = k_lag, bs = \"ts\")",
...
for (pred in weather_predictors) {
    # Smooth baseline
    fit_model_safe(
        sprintf("butterfly_diff_sqrt ~ s(max_butterflies_t_1, k = k_baseline, bs = \\\"ts\\\") + s(lag_duration_hours, k = k_lag, bs = \\\"ts\\\") + s(%s)", pred),
...
for (pair in interaction_pairs) {
    ...
    # Smooth baseline
    fit_model_safe(
        sprintf("butterfly_diff_sqrt ~ s(max_butterflies_t_1, k = k_baseline, bs = \\\"ts\\\") + s(lag_duration_hours, k = k_lag, bs = \\\"ts\\\") + ti(%s, %s)", pred1, pred2),
...
for (pair in interaction_pairs) {
    ...
    fit_model_safe(
        sprintf(
            "butterfly_diff_sqrt ~ s(max_butterflies_t_1, k = k_baseline, bs = \\\"ts\\\") + s(lag_duration_hours, k = k_lag, bs = \\\"ts\\\") + s(%s) + s(%s) + ti(%s, %s)",
...
for (combo in additive_combos) {
    ...
    fit_model_safe(
        sprintf(
            "butterfly_diff_sqrt ~ s(max_butterflies_t_1, k = k_baseline, bs = \\\"ts\\\") + s(lag_duration_hours, k = k_lag, bs = \\\"ts\\\") + %s",
            preds_str
        ),
...
fit_model_safe(
    sprintf(
        "butterfly_diff_sqrt ~ s(max_butterflies_t_1, k = k_baseline, bs = \"ts\") + s(lag_duration_hours, k = k_lag, bs = \"ts\") + %s + %s",
...
# ----------------------------------------------------------------------------
# EXTRA: Linear interaction candidates for parity with 30-min analysis
# ----------------------------------------------------------------------------
cat("\n=== FITTING EXTRA LINEAR INTERACTION CANDIDATES ===\n")
fit_model_safe(
  "butterfly_diff_sqrt ~ s(max_butterflies_t_1, k = k_baseline, bs = \"ts\") + s(lag_duration_hours, k = k_lag, bs = \"ts\") + wind_max_gust + sum_butterflies_direct_sun + wind_max_gust:sum_butterflies_direct_sun",
  "linear", "M77", "Linear interaction: wind_max_gust × sum_butterflies_direct_sun (with baseline + lag duration)"
)
fit_model_safe(
  "butterfly_diff_sqrt ~ s(max_butterflies_t_1, k = k_baseline, bs = \"ts\") + s(lag_duration_hours, k = k_lag, bs = \"ts\") + temp_min + temp_max + wind_max_gust + sum_butterflies_direct_sun + wind_max_gust:sum_butterflies_direct_sun",
  "linear", "M78", "Linear interaction + temp_min + temp_max (with baseline + lag duration)"
)
