...
The [original daily-level analysis](../../reports/daily_gam_results.html) used fixed 6am-6pm weather windows, which Francis identified as having a temporal logic issue:
...
**`butterfly_diff_95th_sqrt`** was selected as the response variable for both analyses based on having the highest Shapiro-Wilk W statistic, indicating best approximation to normality.

This represents the **square root transformation** of the change in 95th percentile butterfly count from day t-1 to day t. The transformation:
...
```{r candidate-table}
# Create empty table template for user to fill in
candidate_vars <- tibble(
  Variable = c(
    ...
    "sum_butterflies_in_sun",
    "sum_butterflies_partial_sun",
    "proportion_butterflies_direct_sun",
    "proportion_butterflies_in_sun",
    "proportion_butterflies_partial_sun",
    "lag_duration_hours",
    "window_start_hour",
    "window_end_hour",
    "metrics_complete"
...
kable(candidate_vars,
      caption = "Candidate Predictor Variables (user to fill in descriptions)")
```
...
![Full Correlation Matrix: All 20 Candidate Predictors](figures/sunset_correlation_full.png){width=100%}
...
```
butterfly_diff_95th_sqrt ~ predictors + s(deployment_id, bs="re")
  + correlation = corAR1(form = ~1|deployment_id)
```

**Key components:**

- **Response**: `butterfly_diff_95th_sqrt` (square root transformed change in 95th percentile count)
...
kable(
  sunset_models %>%
    select(model, description, AICc, delta_AICc, weight_AICc, df,
  ...
  caption = "Top 30 Models: Sunset Window Analysis (ranked by AICc)"
)
```
...
```{r sunset-cv-summary}
# This would ideally load CV results from the analysis
# For now, create placeholder text pointing to full analysis
cat("Cross-validation RMSE for top 5 models:\n\n")
cat("See sunset_window_gam_analysis.html for detailed cross-validation results.\n")
cat("The best model (M31) showed stable out-of-sample performance with RMSE\n")
cat("competitive with or better than more complex models, supporting its selection.\n")
```
...
**Model equation:**
```
butterfly_diff_95th_sqrt ~ te(wind_max_gust, sum_butterflies_direct_sun)
...
![Sunset Window: GAM Check Plots](figures/sunset_gam_diagnostics.png){width=100%}
...
kable(
  sunset_models %>%
    select(model, description, AICc, delta_AICc, weight_AICc,
