...
p_main <- ggplot(model_data, aes(x = wind_max_gust, y = butterfly_diff_95th)) +
  geom_point(alpha = 0.5, size = 2, color = "#4d4d4d") +
  ...
  scale_x_continuous(limits = c(0, max(model_data$wind_max_gust, na.rm = TRUE) * 1.05),
                     expand = c(0, 0)) +
  scale_y_continuous(limits = c(min(model_data$butterfly_diff_95th, na.rm = TRUE) * 1.05,
                                max(model_data$butterfly_diff_95th, na.rm = TRUE) * 1.05),
                     expand = c(0, 0)) +
  labs(
    x = "Maximum wind speed (m/s)",
...
dens_x <- ggplot(model_data, aes(x = wind_max_gust)) +
...
dens_y <- ggplot(model_data, aes(x = butterfly_diff_95th)) +
  geom_density(fill = "#4d4d4d", alpha = 0.4, color = "#4d4d4d", linewidth = 0.5) +
  scale_x_continuous(limits = c(min(model_data$butterfly_diff_95th, na.rm = TRUE) * 1.05,
                                max(model_data$butterfly_diff_95th, na.rm = TRUE) * 1.05),
                     expand = c(0, 0)) +
  theme_void() +
