...
```{r export-normality-visualization}
...
dev.off()
cat("Exported normality visualization to: analysis/reports/figures/response_variable_normality.png\n")
```
...
This section provides a detailed examination of the top 3 models by AIC, including model summaries, partial effects plots, and diagnostic plots for each.

```{r top-3-models-setup}
# Get top 3 models
top_3_models <- aic_results %>%
    slice(1:3)

cat("Top 3 Models:\n")
print(top_3_models %>% select(Model, Correlation, AIC, Delta_AIC, AIC_weight))
```

```{r top-3-detailed-analysis}
#| fig-width: 14
#| fig-height: 12
...
for (i in 1:3) {
    model_name <- top_3_models$Model[i]
    model_obj <- successful_models[[model_name]]
    formula_str <- top_3_models$Formula[i]
    ...
    cat("Correlation:", top_3_models$Correlation[i], "\n")
    cat("AIC:", round(top_3_models$AIC[i], 3), "\n")
    cat("ΔAIC:", round(top_3_models$Delta_AIC[i], 3), "\n")
    cat("AIC Weight:", round(top_3_models$AIC_weight[i], 4), "\n")
    cat("================================================================================\n\n")

    # 1. Model Summary
    cat("### Model Summary ###\n")
    model_summary <- summary(model_obj$gam)
    print(model_summary)
    ...
    top_3_results[[i]] <- list(
        model_name = model_name,
        formula = formula_str,
        correlation = top_3_models$Correlation[i],
        aic = top_3_models$AIC[i],
        delta_aic = top_3_models$Delta_AIC[i],
        aic_weight = top_3_models$AIC_weight[i],
        r_squared = r_squared,
        ...
        tryCatch({
            ...
            if (nrow(smooth_terms) > 0) {
                ...
                if (length(plots) > 0) {
                    ...
                    # Store for export
                    top_3_results[[i]]$partial_effects_plot <- combined_plots
                }
            }
        }, error = function(e) {
            cat("Error creating partial effects plots:", e$message, "\n")
        })
    } else {
        cat("Model uses linear terms only.\n")
        coef_summary <- model_summary$p.table
        print(coef_summary)
    }
    ...
    # Residuals vs Fitted
    plot(model_obj$lme, main = paste("Model", i, ": Residuals vs Fitted"))

    # Q-Q plot
    qqnorm(residuals(model_obj$lme, type = "normalized"),
           main = paste("Model", i, ": Q-Q Plot"))
    qqline(residuals(model_obj$lme, type = "normalized"))

    # Scale-location plot
    plot(fitted(model_obj$lme),
         sqrt(abs(residuals(model_obj$lme, type = "normalized"))),
         main = paste("Model", i, ": Scale-Location"),
         xlab = "Fitted values",
         ylab = "sqrt(|Standardized residuals|)")
    lines(lowess(fitted(model_obj$lme),
                 sqrt(abs(residuals(model_obj$lme, type = "normalized")))),
          col = "red")

    # Histogram of residuals
    hist(residuals(model_obj$lme, type = "normalized"),
         breaks = 30,
         main = paste("Model", i, ": Residual Distribution"),
         xlab = "Standardized Residuals",
         col = "lightblue")
...
```{r export-top-3-models}
# Export summaries for top 3 models
top_3_summary <- tibble(
    Rank = 1:3,
    Model = sapply(top_3_results, function(x) x$model_name),
    Formula = sapply(top_3_results, function(x) x$formula),
    Correlation = sapply(top_3_results, function(x) x$correlation),
    AIC = sapply(top_3_results, function(x) round(x$aic, 3)),
    Delta_AIC = sapply(top_3_results, function(x) round(x$delta_aic, 3)),
    AIC_Weight = sapply(top_3_results, function(x) round(x$aic_weight, 4)),
    R_squared = sapply(top_3_results, function(x) round(x$r_squared, 4)),
    Deviance_Explained = sapply(top_3_results, function(x) round(x$dev_explained * 100, 2))
)

write_csv(top_3_summary,
          here("analysis", "reports", "figures", "top_3_models_summary.csv"))

cat("Exported top 3 models summary to: analysis/reports/figures/top_3_models_summary.csv\n")

# Export partial effects plots for each of top 3 models
for (i in 1:3) {
    if (!is.null(top_3_results[[i]]$partial_effects_plot)) {
        ggsave(
            here("analysis", "reports", "figures",
                 paste0("top_3_model_", i, "_partial_effects.png")),
            plot = top_3_results[[i]]$partial_effects_plot,
            width = 14, height = 8, dpi = 300
        )
        cat("Exported Model", i, "partial effects to: analysis/reports/figures/top_3_model_", i, "_partial_effects.png\n")
    }
}

# Export diagnostic plots for each of top 3 models
for (i in 1:3) {
    model_name <- top_3_models$Model[i]
    model_obj <- successful_models[[model_name]]

    png(here("analysis", "reports", "figures",
             paste0("top_3_model_", i, "_diagnostics.png")),
        width = 12, height = 10, units = "in", res = 300)
    par(mfrow = c(2, 2))
    ...
    cat("Exported Model", i, "diagnostics to: analysis/reports/figures/top_3_model_", i, "_diagnostics.png\n")
}

# Export model summary text for each of top 3 models
for (i in 1:3) {
    model_name <- top_3_models$Model[i]
    model_obj <- successful_models[[model_name]]

    # Capture summary output as text
    summary_text <- capture.output(summary(model_obj$gam))

    # Write to text file
    writeLines(
        summary_text,
        here("analysis", "reports", "figures",
             paste0("top_3_model_", i, "_summary.txt"))
    )

    cat("Exported Model", i, "summary to: analysis/reports/figures/top_3_model_", i, "_summary.txt\n")
}
```
