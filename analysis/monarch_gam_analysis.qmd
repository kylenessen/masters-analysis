...
add_zero_line <- function(plot) {
  # Insert the zero line as the first layer so it appears behind everything
  zero_line_layer <- geom_hline(yintercept = 0, color = "gray50", size = 0.8, alpha = 1)
  
  # Get existing layers
  existing_layers <- plot$layers
  
  # Clear layers and add zero line first, then add back existing layers
  plot$layers <- NULL
  plot <- plot + zero_line_layer
  
  # Add back all existing layers on top
...
# Set custom y-axis limits as requested
y_limits <- c(-8, 2)
y_breaks <- seq(-8, 2, by = 2)

cat("Custom y-axis limits:", y_limits, "\n")
...
# Override the default colors using a more direct approach
...
# Override colors using comprehensive layer modification
