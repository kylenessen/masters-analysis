...
model_specs <- list(
    ...
    # NEW: Tensor-product interaction candidates (wind × sun) for parity with daily analysis
...
# Record available smooth terms and whether expected smooths are present
available_smooths <- rownames(summary(best_model$gam)$s.table)
expected_smooths <- c(
  "s(total_butterflies_t_lag)",
  "s(temperature_avg)",
  "s(butterflies_direct_sun_t_lag)",
  "s(time_within_day_t)"
)
has_expected <- all(expected_smooths %in% available_smooths)
cat("Has expected smooths:", has_expected, "\n")
```
...
if (isTRUE(has_expected)) {
  ...
  # Fallback axis settings when expected smooths are not present
  y_limits <- c(-2, 1.5)
  y_breaks <- seq(-2, 1.5, by = 1)
  cat("Calculated y-axis limits (default):", y_limits, "\n")
}
```

```{r create-individual-plots}
# Skip detailed effect plots if expected smooths are missing
if (!isTRUE(has_expected)) {
  cat("Skipping detailed effect plots: expected smooths not present in best model\n")
} else {
...
# Add flight threshold area (12.7-16°C) with fuzzy edges AFTER color modifications
# Create a gradient effect by layering rectangles with decreasing alpha
fade_width <- 0.5  # Width of fade zone on each side

# Core threshold area (solid)
p4 <- p4 +
  annotate("rect",
...
# Fade zones - multiple layers for smooth gradient
fade_steps <- 5
for (i in 1:fade_steps) {
  alpha_val <- 0.4 * (fade_steps - i + 1) / fade_steps
  fade_offset <- fade_width * i / fade_steps

  # Left fade
  p4 <- p4 +
    annotate("rect",
             xmin = 12.7 + fade_width - fade_offset,
             xmax = 12.7 + fade_width - fade_offset + fade_width/fade_steps,
             ymin = -Inf, ymax = Inf,
             fill = "#ADD8E6", alpha = alpha_val)

  # Right fade
  p4 <- p4 +
    annotate("rect",
             xmin = 16 - fade_width + fade_offset - fade_width/fade_steps,
             xmax = 16 - fade_width + fade_offset,
             ymin = -Inf, ymax = Inf,
             fill = "#ADD8E6", alpha = alpha_val)
}
...
if (isTRUE(has_expected) && exists("p1") && exists("p2") && exists("p3") && exists("p4")) {
  ...
  cat("Skipping combined 1x4 plot: effects unavailable\n")
}
```
...
<!-- Power analysis moved to analysis/monarch_gam_power_analysis.R to keep renders fast -->
...
Sensitivity analyses focused on threshold-based wind metrics are documented separately in `analysis/monarch_threshold_analysis.qmd`.
...
thesis_results_fig_dir <- here("..", "masters-thesis", "figures", "results")
...
if (!dir.exists(thesis_results_fig_dir)) dir.create(thesis_results_fig_dir, recursive = TRUE)
if (!dir.exists(thesis_results_tab_dir)) dir.create(thesis_results_tab_dir, recursive = TRUE)
...
### Wind × Sun Interaction (Binned, High-Res)

```{r export-binned-interaction}
# Source helper for consistent binned interaction plots
source(here("analysis", "plot_binned_interaction.R"))

# Ensure export directories exist when running this chunk standalone
if (!exists("fig_dir")) {
  export_dir <- here("thesis_exports")
  fig_dir <- file.path(export_dir, "figures")
  if (!dir.exists(export_dir)) dir.create(export_dir, recursive = TRUE)
...
if (!exists("thesis_results_fig_dir")) {
  thesis_results_fig_dir <- here("..", "masters-thesis", "figures", "results")
}
if (!dir.exists(thesis_results_fig_dir)) {
  dir.create(thesis_results_fig_dir, recursive = TRUE)
}

# Check if the best model includes the wind × sun tensor product
has_wind_sun_ti <- tryCatch({
  any(grepl(
    "ti\\(max_gust,butterflies_direct_sun_t_lag\\)",
    rownames(summary(best_model$gam)$s.table)
  ))
}, error = function(e) FALSE)

if (isTRUE(has_wind_sun_ti)) {
  # Create the binned, high-resolution interaction surface plot
  p_inter_binned <- create_binned_interaction_plot(
    gam_model = best_model$gam,
    x_var = "max_gust",
    y_var = "butterflies_direct_sun_t_lag",
    data = model_data,
    title = "Wind × Sun Interaction",
    subtitle = paste0(
      "Binned visualization | Best model ", best_model_name,
      " | AIC = ", sprintf("%.1f", AIC(best_model$lme))
    ),
    xlab = "Maximum wind speed (m/s)",
    ylab = "Butterflies in direct sun",
    n = 400,
    # More categories with fine resolution near zero
    limits = c(-6, 6),
    nbreaks = 17,
  ...
  # Add threshold reference line at 2 m/s
  p_inter_binned <- p_inter_binned +
    geom_vline(xintercept = 2, color = "red", linetype = "dashed", size = 0.8, alpha = 0.8)

  print(p_inter_binned)

  # Save high-resolution copies
  out_file <- file.path(fig_dir, "interaction_wind_x_sun_binned.png")
  ggsave(out_file, p_inter_binned, width = 7, height = 6, dpi = 300, bg = "white")
  cat("Binned interaction plot saved to:", out_file, "\n")

  # Also save to thesis repo if path exists
  if (exists("thesis_results_fig_dir") && dir.exists(thesis_results_fig_dir)) {
    out_file_thesis <- file.path(thesis_results_fig_dir, "interaction_wind_x_sun_binned.png")
    ggsave(out_file_thesis, p_inter_binned, width = 7, height = 6, dpi = 300, bg = "white")
    cat("Also saved to:", out_file_thesis, "\n")
  }
} else {
  cat("Best model does not include ti(max_gust, butterflies_direct_sun_t_lag); skipping interaction surface plot.\n")
}
```
...
create_readable_terms <- function(formula_str) {
    ...
    formula_clean <- gsub("butterfly_difference_cbrt ~ ", "", formula_str, fixed = TRUE)

    # Split into terms on literal '+'
    original_terms <- trimws(strsplit(formula_clean, "+", fixed = TRUE)[[1]])
    original_terms <- original_terms[original_terms != ""]

    # Process each term individually
    formatted_terms <- vapply(original_terms, function(term) {
        term <- trimws(term)

        # Detect smooths by prefix 's(' and tensor interactions 'ti('
        is_smooth <- startsWith(term, "s(") || startsWith(term, "ti(") || startsWith(term, "te(")

        # Helper to humanize variable names
        humanize <- function(x) {
            dplyr::case_when(
        ...
        if (startsWith(term, "s(") && endsWith(term, ")")) {
            inner <- substring(term, 3, nchar(term) - 1)
            readable_name <- humanize(inner)
            return(paste0("\\textbullet\\ ", readable_name))
        }
        if ((startsWith(term, "ti(") || startsWith(term, "te(")) && endsWith(term, ")")) {
            inner <- substring(term, 4, nchar(term) - 1)
            parts <- trimws(strsplit(inner, ",", fixed = TRUE)[[1]])
            parts_human <- vapply(parts, humanize, character(1))
            inter_name <- paste0(substr(term, 1, 2), "(", paste(parts_human, collapse = ", "), ")")
            return(paste0("\\textbullet\\ ", inter_name))
        }

        # Fallback: treat as linear term
        var_name <- term

        # Replace with readable names
        readable_name <- humanize(var_name)

        # Format with bullet and linear notation if needed
        paste0("\\textbullet\\ ", readable_name, " (linear)")
    }, character(1))
    ...
    # Join with LaTeX line breaks and do not escape in kable
...
# Also copy to thesis repository tables directory
try({
  file.copy(file.path(tab_dir, "model_selection_table.csv"),
            file.path(thesis_results_tab_dir, "model_selection_table.csv"), overwrite = TRUE)
  file.copy(file.path(tab_dir, "model_selection_table.tex"),
            file.path(thesis_results_tab_dir, "model_selection_table.tex"), overwrite = TRUE)
})
```
...
# Also copy to thesis repository tables directory
try({
  file.copy(file.path(tab_dir, "smooth_terms_summary.csv"),
            file.path(thesis_results_tab_dir, "smooth_terms_summary.csv"), overwrite = TRUE)
  file.copy(file.path(tab_dir, "smooth_terms_summary.tex"),
            file.path(thesis_results_tab_dir, "smooth_terms_summary.tex"), overwrite = TRUE)
})
```
...
# Skip if expected smooths are missing in best model
if (!isTRUE(has_expected)) {
  cat("Skipping partial effect plot exports: expected smooths not present in best model\n")
} else {
...
# Create 2x2-specific plots with proper y-axis sharing
pe1_2x2 <- pe1 + labs(title = NULL)  # Top-left: remove title, keep y-axis label

pe2_2x2 <- pe2 + labs(title = NULL)  # Top-right: remove title, no y-axis

pe3_2x2 <- draw(best_model$gam, select = "s(time_within_day_t)", 
           rug = FALSE, residuals = FALSE) +
  labs(
    title = NULL,  # No title
    x = "Time since sunrise (minutes)",
    y = "Partial effect"  # Bottom-left: keep y-axis label
  ) +
  scale_y_continuous(limits = y_limits, breaks = y_breaks) +
  pub_theme_grid +
  theme(plot.caption = element_blank())

pe3_2x2 <- add_zero_line(pe3_2x2)

# Apply colors to pe3_2x2
for (i in seq_along(pe3_2x2$layers)) {
  if (!is.null(pe3_2x2$layers[[i]]$geom) && inherits(pe3_2x2$layers[[i]]$geom, "GeomHline")) {
    next
  }
  if ("colour" %in% names(pe3_2x2$layers[[i]]$aes_params)) {
    pe3_2x2$layers[[i]]$aes_params$colour <- plot_colors[3]
  }
  if ("fill" %in% names(pe3_2x2$layers[[i]]$aes_params)) {
    pe3_2x2$layers[[i]]$aes_params$fill <- plot_colors_light[3]
  }
  if (!is.null(pe3_2x2$layers[[i]]$geom)) {
    if (inherits(pe3_2x2$layers[[i]]$geom, "GeomLine")) {
      pe3_2x2$layers[[i]]$aes_params$colour <- plot_colors[3]
    }
    if (inherits(pe3_2x2$layers[[i]]$geom, c("GeomRibbon", "GeomPolygon"))) {
      pe3_2x2$layers[[i]]$aes_params$fill <- plot_colors_light[3]
    }
  }
}

pe4_2x2 <- draw(best_model$gam, select = "s(temperature_avg)", 
           rug = FALSE, residuals = FALSE) +
  labs(
    title = NULL,  # No title
    x = "Temperature (°C)",
    y = ""  # Bottom-right: no y-axis label
  ) +
  scale_y_continuous(limits = y_limits, breaks = y_breaks) +
  pub_theme_grid +
  theme(
...
pe4_2x2 <- add_zero_line(pe4_2x2)

# Apply colors to pe4_2x2
for (i in seq_along(pe4_2x2$layers)) {
  if (!is.null(pe4_2x2$layers[[i]]$geom) && inherits(pe4_2x2$layers[[i]]$geom, "GeomHline")) {
    next
  }
  if ("colour" %in% names(pe4_2x2$layers[[i]]$aes_params)) {
    pe4_2x2$layers[[i]]$aes_params$colour <- plot_colors[4]
  }
  if ("fill" %in% names(pe4_2x2$layers[[i]]$aes_params)) {
    pe4_2x2$layers[[i]]$aes_params$fill <- plot_colors_light[4]
  }
  if (!is.null(pe4_2x2$layers[[i]]$geom)) {
    if (inherits(pe4_2x2$layers[[i]]$geom, "GeomLine")) {
      pe4_2x2$layers[[i]]$aes_params$colour <- plot_colors[4]
    }
    if (inherits(pe4_2x2$layers[[i]]$geom, c("GeomRibbon", "GeomPolygon"))) {
      pe4_2x2$layers[[i]]$aes_params$fill <- plot_colors_light[4]
    }
  }
}

# Add flight threshold area (12.7-16°C) with fuzzy edges to pe4_2x2 AFTER color modifications
# Create a gradient effect by layering rectangles with decreasing alpha
fade_width <- 0.5  # Width of fade zone on each side
...
# Create and export 2x2 combined figure with proper y-axis sharing
tryCatch({
  combined_pub_2x2 <- wrap_plots(pe1_2x2, pe2_2x2, pe3_2x2, pe4_2x2, nrow = 2, ncol = 2)
...
# Mirror key figures to thesis repo figures directory
try({
  if (!dir.exists(thesis_results_fig_dir)) dir.create(thesis_results_fig_dir, recursive = TRUE)
  fig_files <- c(
    "effect_previous_butterfly_count_final.png",
    "effect_temperature_final.png",
    "effect_butterflies_direct_sun_final.png",
    "diurnal_patterns_final.png",
    "combined_partial_effects_2x2_final.png",
    "combined_partial_effects_1x4_final.png"
  )
  for (f in fig_files) {
    src <- file.path(fig_dir, f)
    if (file.exists(src)) file.copy(src, file.path(thesis_results_fig_dir, f), overwrite = TRUE)
  }
})

# Export standalone partial effect for maximum wind speed from model M24
if ("M24" %in% names(successful_models)) {
  model_m24 <- successful_models[["M24"]]
  wind_color <- "#4C8BB9"
  wind_color_light <- lighten_color(wind_color)

  pe_wind_m24 <- draw(model_m24$gam, select = "s(max_gust)", rug = FALSE, residuals = FALSE) +
    labs(
      title = NULL,
      x = "Maximum wind speed (m/s)",
      y = "Partial effect"
  ...
  pe_wind_m24 <- add_zero_line(pe_wind_m24)

  for (i in seq_along(pe_wind_m24$layers)) {
    if (!is.null(pe_wind_m24$layers[[i]]$geom) && inherits(pe_wind_m24$layers[[i]]$geom, "GeomHline")) {
      next
    }

    if ("colour" %in% names(pe_wind_m24$layers[[i]]$aes_params)) {
      pe_wind_m24$layers[[i]]$aes_params$colour <- wind_color
    }
    if ("fill" %in% names(pe_wind_m24$layers[[i]]$aes_params)) {
      pe_wind_m24$layers[[i]]$aes_params$fill <- wind_color_light
    }
    if (!is.null(pe_wind_m24$layers[[i]]$geom)) {
      if (inherits(pe_wind_m24$layers[[i]]$geom, "GeomLine")) {
        pe_wind_m24$layers[[i]]$aes_params$colour <- wind_color
      }
      if (inherits(pe_wind_m24$layers[[i]]$geom, c("GeomRibbon", "GeomPolygon"))) {
        pe_wind_m24$layers[[i]]$aes_params$fill <- wind_color_light
      }
    }
  }

  pe_wind_m24 <- pe_wind_m24 +
    geom_vline(xintercept = 2, color = "red", linetype = "dashed", size = 0.8)

  ggsave(file.path(fig_dir, "m24_wind_partial_effect.png"), pe_wind_m24,
      width = 6, height = 5, dpi = 300, bg = "white"
  )

  thesis_wind_path <- file.path(thesis_results_fig_dir, "m24_wind_partial_effect.png")
  ggsave(thesis_wind_path, pe_wind_m24,
      width = 6, height = 5, dpi = 300, bg = "white"
  )

  cat("M24 wind partial effect exported successfully\n")
  cat("- Saved to thesis_exports/figures/m24_wind_partial_effect.png\n")
  cat("- Saved to ../masters-thesis/figures/results/m24_wind_partial_effect.png\n")
} else {
  cat("Model M24 unavailable; skipping wind partial effect export\n")
}
...
cat("- m24_wind_partial_effect.png (also saved to ../masters-thesis/figures/results/)\n")
...
### Publication-Ready Partial Effects (Smart 1x3 without Sun)

```{r export-partial-effects-smart}
# Build partial effects for the smooths present in best model (exclude direct sun)
library(gratia)
library(patchwork)

# Identify smooth labels present
available_smooths <- rownames(summary(best_model$gam)$s.table)
lab_prev <- available_smooths[grepl("^s\\(total_butterflies_t_lag\\)", available_smooths)]
lab_temp <- available_smooths[grepl("^s\\(temperature_avg\\)", available_smooths)]
lab_time <- available_smooths[grepl("^s\\(time_within_day_t\\)", available_smooths)]

# Colors
col_prev <- "#9673c5"  # purple
col_time <- "#79a44c"  # green
col_temp <- "#b86e7e"  # mauve/pink
lighten <- function(hex, amount = 0.1) {
  rgb_vals <- col2rgb(hex)
  lightened <- rgb_vals + (255 - rgb_vals) * amount
  rgb(lightened[1]/255, lightened[2]/255, lightened[3]/255)
}

# Helper to recolor gratia::draw plot
recolor <- function(p, col_line, col_fill) {
  # Add the neutral zero line first
  p$layers <- c(list(geom_hline(yintercept = 0, color = "gray70", size = 0.8)), p$layers)
  for (i in seq_along(p$layers)) {
    if (!is.null(p$layers[[i]]$geom) && inherits(p$layers[[i]]$geom, "GeomHline")) next
    if ("colour" %in% names(p$layers[[i]]$aes_params)) p$layers[[i]]$aes_params$colour <- col_line
    if ("fill" %in% names(p$layers[[i]]$aes_params)) p$layers[[i]]$aes_params$fill <- col_fill
    if (!is.null(p$layers[[i]]$geom)) {
      if (inherits(p$layers[[i]]$geom, "GeomLine")) p$layers[[i]]$aes_params$colour <- col_line
      if (inherits(p$layers[[i]]$geom, c("GeomRibbon", "GeomPolygon"))) p$layers[[i]]$aes_params$fill <- col_fill
    }
  }
  p
}
...
if (length(lab_prev) == 1) {
  p_prev <- draw(best_model$gam, select = lab_prev, rug = FALSE, residuals = FALSE) +
    labs(title = "Previous Butterfly Count", x = "Previous butterfly count", y = "Partial effect") +
    theme_classic(base_size = 12) + theme(plot.caption = element_blank())
  plots$prev <- recolor(p_prev, col_prev, lighten(col_prev))
  rr <- ggplot_build(plots$prev)$layout$panel_scales_y[[1]]$range$range
  ymins <- c(ymins, rr[1]); ymaxs <- c(ymaxs, rr[2])
}
if (length(lab_time) == 1) {
  p_time <- draw(best_model$gam, select = lab_time, rug = FALSE, residuals = FALSE) +
    labs(title = "Time Since Sunrise", x = "Time since sunrise (minutes)", y = "Partial effect") +
    theme_classic(base_size = 12) + theme(plot.caption = element_blank())
  plots$time <- recolor(p_time, col_time, lighten(col_time))
  rr <- ggplot_build(plots$time)$layout$panel_scales_y[[1]]$range$range
  ymins <- c(ymins, rr[1]); ymaxs <- c(ymaxs, rr[2])
}
if (length(lab_temp) == 1) {
  p_temp <- draw(best_model$gam, select = lab_temp, rug = FALSE, residuals = FALSE) +
    labs(title = "Temperature", x = "Temperature (°C)", y = "Partial effect") +
    theme_classic(base_size = 12) + theme(plot.caption = element_blank())
  plots$temp <- recolor(p_temp, col_temp, lighten(col_temp))
  rr <- ggplot_build(plots$temp)$layout$panel_scales_y[[1]]$range$range
  ymins <- c(ymins, rr[1]); ymaxs <- c(ymaxs, rr[2])
}

if (length(plots) > 0) {
  y_min <- floor(min(ymins)); y_max <- 1.5
  fix_y <- function(p) p + scale_y_continuous(limits = c(y_min, y_max), breaks = seq(y_min, y_max, by = 1))
  plots <- lapply(plots, fix_y)

  # Compose 1x3 layout in preferred order: previous, time, temperature
  combo <- wrap_plots(plots$prev, plots$time, plots$temp, nrow = 1, ncol = length(plots))
  out1x3 <- file.path(fig_dir, "combined_partial_effects_1x3_final.png")
  ggsave(out1x3, combo, width = max(12, 4 * length(plots)), height = 5, dpi = 300, bg = "white")
  cat("1x3 partial effects exported to:", out1x3, "\n")

  # Mirror to thesis repo
  try({
    if (!dir.exists(thesis_results_fig_dir)) dir.create(thesis_results_fig_dir, recursive = TRUE)
    file.copy(out1x3, file.path(thesis_results_fig_dir, basename(out1x3)), overwrite = TRUE)
  })
} else {
  cat("No partial effects to export for current best model.\n")
}
```
...
# Mirror diagnostics to thesis repo
try({
  if (!dir.exists(thesis_results_fig_dir)) dir.create(thesis_results_fig_dir, recursive = TRUE)
  diag_files <- c("residuals_vs_fitted.png", "qq_plot.png", "combined_diagnostics.png")
  for (f in diag_files) {
    src <- file.path(fig_dir, f)
    if (file.exists(src)) file.copy(src, file.path(thesis_results_fig_dir, f), overwrite = TRUE)
  }
...
try({
  if (!dir.exists(thesis_results_fig_dir)) dir.create(thesis_results_fig_dir, recursive = TRUE)
  src <- file.path(fig_dir, "wind_hypothesis_scatter.png")
  if (file.exists(src)) file.copy(src, file.path(thesis_results_fig_dir, "wind_hypothesis_scatter.png"), overwrite = TRUE)
})
```
...
<!-- Power analysis table export moved to analysis/monarch_gam_power_analysis.R -->
...
## Mirror exports to thesis repository (figures + tables)
try({
  # Ensure dirs exist
  if (!dir.exists(thesis_results_fig_dir)) dir.create(thesis_results_fig_dir, recursive = TRUE)
  if (!dir.exists(thesis_results_tab_dir)) dir.create(thesis_results_tab_dir, recursive = TRUE)
  ...
  fig_list <- c(
    ...
    "effect_butterflies_direct_sun_final.png",
    "diurnal_patterns_final.png",
    "combined_diagnostics.png",
    "wind_hypothesis_scatter.png",
    "m24_wind_partial_effect.png",
    "interaction_wind_x_sun_binned.png"
  )
  for (f in fig_list) {
    src <- file.path(fig_dir, f)
    if (file.exists(src)) file.copy(src, file.path(thesis_results_fig_dir, f), overwrite = TRUE)
  ...
  # Tables to mirror
  tab_list <- c(
    "model_selection_table.csv", "model_selection_table.tex",
    "smooth_terms_summary.csv", "smooth_terms_summary.tex",
    "data_summary.csv", "data_summary.tex"
  )
  for (t in tab_list) {
    src <- file.path(tab_dir, t)
    if (file.exists(src)) file.copy(src, file.path(thesis_results_tab_dir, t), overwrite = TRUE)
  }
})
