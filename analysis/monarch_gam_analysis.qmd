...
p3 <- ggplot(model_data, aes(x = max_gust, y = minutes_above_threshold)) +
    ...
    labs(
        title = "Relationship: Max Gust vs Minutes Above Threshold",
...
create_readable_terms <- function(formula_str) {
    ...
    formatted_terms <- sapply(original_terms, function(term) {
        ...
        readable_name <- case_when(
            var_name == "total_butterflies_t_lag" ~ "Lagged Total Butterflies",
            var_name == "max_gust" ~ "Max Gust",
            var_name == "temperature_avg" ~ "Average Temperature",
            var_name == "butterflies_direct_sun_t_lag" ~ "Butterflies in Direct Sunlight",
            var_name == "time_within_day_t" ~ "Minutes Since Dawn",
            TRUE ~ var_name
        )
