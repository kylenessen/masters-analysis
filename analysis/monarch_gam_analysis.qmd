...
# Define custom theme with grid lines
...
y_limits <- c(min(y_ranges[1,]) * 1.1, max(y_ranges[2,]) * 1.1)

cat("Consistent y-axis limits:", round(y_limits, 3), "\n")
```

```{r create-individual-plots}
# Define colors for each variable
plot_colors <- c(
  "steelblue",    # Previous butterfly count
  "firebrick",    # Temperature
  "darkorange",   # Butterflies in direct sun
  "forestgreen"   # Time since sunrise
)

# Create individual plots with consistent y-axis and unique colors
# Note: We'll use coord_cartesian instead of ylim to avoid clipping data
p1 <- draw(best_model$gam, select = "s(total_butterflies_t_lag)", 
...
# Override the default colors by modifying the plot layers
if (length(p1$layers) >= 2) {
  p1$layers[[1]]$aes_params$colour <- plot_colors[1]  # smooth line
  p1$layers[[2]]$aes_params$fill <- alpha(plot_colors[1], 0.3)  # confidence band
}

p2 <- draw(best_model$gam, select = "s(temperature_avg)", 
           rug = FALSE, residuals = FALSE) +
  labs(
    title = "Temperature",
    x = "Temperature (°C)",
...
if (length(p2$layers) >= 2) {
  p2$layers[[1]]$aes_params$colour <- plot_colors[2]
  p2$layers[[2]]$aes_params$fill <- alpha(plot_colors[2], 0.3)
}

p3 <- draw(best_model$gam, select = "s(butterflies_direct_sun_t_lag)", 
           rug = FALSE, residuals = FALSE) +
  labs(
...
if (length(p3$layers) >= 2) {
  p3$layers[[1]]$aes_params$colour <- plot_colors[3]
  p3$layers[[2]]$aes_params$fill <- alpha(plot_colors[3], 0.3)
}

p4 <- draw(best_model$gam, select = "s(time_within_day_t)", 
           rug = FALSE, residuals = FALSE) +
  labs(
...
if (length(p4$layers) >= 2) {
  p4$layers[[1]]$aes_params$colour <- plot_colors[4]
  p4$layers[[2]]$aes_params$fill <- alpha(plot_colors[4], 0.3)
}
...
```{r combined-effects-2x2}
#| fig-width: 12
#| fig-height: 10
# Check that all plots exist before combining
if (exists("p1") && exists("p2") && exists("p3") && exists("p4")) {
  # Create 2x2 combined plot with explicit layout
  combined_2x2 <- (p1 + p2) / (p3 + p4) + 
    plot_layout(ncol = 2, nrow = 2) +
    plot_annotation(
      title = "Partial Effects from Best GAM Model (2x2 Layout)",
...
if (exists("p1") && exists("p2") && exists("p3") && exists("p4")) {
  ...
  combined_1x4 <- wrap_plots(p1, p2, p3, p4, nrow = 1, ncol = 4) +
    plot_annotation(
      title = "Partial Effects from Best GAM Model (1x4 Layout)",
      theme = theme(plot.title = element_text(size = 16, hjust = 0.5, face = "bold"))
    )
...
# Define publication theme with grid lines (no horizontal lines)
...
pe1 <- draw(best_model$gam, select = "s(total_butterflies_t_lag)", 
           rug = FALSE, residuals = FALSE) +
  labs(
    title = "Previous Butterfly Count",
    x = "Previous butterfly count",
  ...
  scale_fill_manual(values = alpha(plot_colors[1], 0.3)) +
  ylim(y_limits) +
  pub_theme_grid

pe2 <- draw(best_model$gam, select = "s(temperature_avg)", 
           rug = FALSE, residuals = FALSE) +
  labs(
    title = "Temperature",
    x = "Temperature (°C)",
  ...
  scale_fill_manual(values = alpha(plot_colors[2], 0.3)) +
  ylim(y_limits) +
  pub_theme_grid

pe3 <- draw(best_model$gam, select = "s(butterflies_direct_sun_t_lag)", 
           rug = FALSE, residuals = FALSE) +
  labs(
    title = "Butterflies in Direct Sun",
    x = "Butterflies in direct sun",
  ...
  scale_fill_manual(values = alpha(plot_colors[3], 0.3)) +
  ylim(y_limits) +
  pub_theme_grid

pe4 <- draw(best_model$gam, select = "s(time_within_day_t)", 
           rug = FALSE, residuals = FALSE) +
  labs(
    title = "Time Since Sunrise",
    x = "Time since sunrise (minutes)",
  ...
  scale_fill_manual(values = alpha(plot_colors[4], 0.3)) +
  ylim(y_limits) +
  pub_theme_grid
...
tryCatch({
  combined_pub_2x2 <- (pe1 + pe2) / (pe3 + pe4) + 
    plot_layout(ncol = 2, nrow = 2) +
    plot_annotation(
      title = "Partial Effects from Best GAM Model",
      theme = theme(plot.title = element_text(size = 16, hjust = 0.5, face = "bold"))
    )
...
tryCatch({
  combined_pub_1x4 <- wrap_plots(pe1, pe2, pe3, pe4, nrow = 1, ncol = 4) +
    plot_annotation(
      title = "Partial Effects from Best GAM Model",
      theme = theme(plot.title = element_text(size = 16, hjust = 0.5, face = "bold"))
    )
