...
This analysis examines how previous day weather conditions affect day-to-day changes in overwintering monarch butterfly abundance using Generalized Additive Mixed Models (GAMMs). The analysis was motivated by field observations suggesting that monarchs respond to wind by relocating to more sheltered positions, and by a need to examine effects over longer time horizons than the 30-minute interval analysis.

**Key Findings:**

- The analysis tested 105 models examining both absolute and proportional (density-dependent) effects of environmental variables
- Models including previous day's butterfly count as a baseline performed substantially better (ΔAIC > 50)
- The best-supported models indicate complex, non-linear relationships between environmental variables and butterfly abundance changes
- Wind effects are present but show unexpected patterns that warrant careful interpretation

## 1. Motivation and Background

### Why a Daily-Level Analysis?

The impetus for this analysis came from discussions with Jessica Griffiths, who expressed skepticism about the absence of wind effects in the 30-minute interval analysis. Both she and I have observed monarchs at Pismo relocating in response to wind—specifically moving to the leeward side of trees when exposed to strong south winds. These observations, combined with Kingston Leong's documented observations of wind effects, suggested the need for a different analytical approach.

**Key rationale:**

1. **Longer time horizons**: Day-to-day changes may capture more durable behavioral responses (persistence vs. emigration) than instantaneous counts
2. **Reduced temporal noise**: Using daily peak counts removes confounding effects of diurnal movement patterns (morning dispersal, afternoon cluster reformation)
3. **Different response mechanisms**: Unfavorable conditions on day *t-1* may lead to emigration detectable on day *t*

### Why Use Peak Counts?

Rather than analyzing counts at arbitrary time points, we use the 95th percentile of each day's counts. This approach:
...
We examine previous day conditions affecting next-day abundance because:

- This represents a plausible biological response window
- Longer time horizons introduce uncertainty—we cannot track individual butterflies, only count populations
- Beyond 24 hours, we lose confidence that we're observing the same population that experienced the initial conditions
...
Selecting an appropriate response variable is critical for valid statistical inference. We systematically evaluated multiple candidate response variables and transformations to identify the distribution that best meets model assumptions.

#### Response Variable Candidates

We considered three measures of daily abundance change:
...
For each candidate, we tested two transformations:
- **Original**: Untransformed difference
- **Square root**: Sign-preserving square root transformation

#### Normality Assessment

We assessed normality using multiple criteria:
- **Skewness**: Deviation from symmetry (target: close to 0)
- **Kurtosis**: Tail heaviness (target: close to 0 for normal)
- **Shapiro-Wilk test**: Formal test of normality (higher p-value better)
...
**Normality score: 0.81** (highest among all candidates)

**Properties:**
- **Sign-preserving transformation**: Maintains direction of change
  - Positive values = increase in abundance
  - Negative values = decrease in abundance
  - Zero = no change
- **Symmetry**: Skewness closer to 0 than untransformed
- **Variance stabilization**: Reduces heteroscedasticity
- **Robustness**: 95th percentile less sensitive to extreme outliers than maximum
- **Interpretability**: Square root scale is reasonably intuitive

**Why this matters:** Using a more normally distributed response variable improves:
- Validity of confidence intervals and p-values
- Model residual behavior
- Reliability of predictions

The square root transformation is a common variance-stabilizing transformation for count-based data, and the sign-preserving version allows us to model both increases and decreases in a symmetric way.

## 3. Candidate Predictor Variables

This analysis generated numerous potential environmental predictors characterizing full-day weather patterns. The table below describes all variables initially considered for model inclusion.
...
butterfly_vars <- tibble(
    Variable = c(
        ...
        "butterflies_top3_mean_t_1", "sum_butterflies_direct_sun_t_1"
    ),
    Description = c(
        ...
        "Sum of all butterflies observed in direct sunlight throughout previous day (proxy for sun exposure)"
    ),
    Units = c("count", "count", "count", "cumulative count"),
    Role = c("Baseline", "Baseline", "Baseline", "Predictor")
)

kable(butterfly_vars, caption = "Butterfly abundance metrics from previous day")
```

### 3.2 Temperature Metrics (Previous Day)
...
**Note:** Many temperature metrics are highly correlated (>0.75). Models tested various combinations to identify which best explain abundance changes.
...
**Wind variable selection rationale:**

- Most wind metrics are highly correlated with each other (see Section 3.4)
- Standard deviation of wind showed low correlation (r=0.76 with max gust) but was excluded for simplicity
- Chose `wind_max_gust_t_1` for biological interpretability and consistency with 30-minute interval analysis
- Threshold-based metrics (minutes above 2 m/s, etc.) were excluded due to high correlation with max gust

### 3.4 Correlation Matrix: All Candidate Predictors

This correlation matrix shows all potential fixed effects initially considered. High correlations (|r| > 0.75) indicate redundancy between variables.
...
**Key observations from full correlation matrix:**

1. **Temperature metrics** are highly intercorrelated (r > 0.80):
   - temp_max, temp_mean, temp_at_max_count, hours_above_15C, degree_hours_above_15C
   - temp_min shows moderate correlation with other temperature metrics

2. **Wind metrics** are highly intercorrelated (r > 0.75):
   - wind_max_gust, wind_gust_sum, wind_gust_hours, wind_minutes_above_2ms
   - All measure similar aspects of wind exposure with different emphases

3. **Between-category correlations**:
   - Negative correlation between wind metrics and temperature metrics (r ≈ -0.4 to -0.5)
   - Suggests windy days tend to be cooler

4. **Butterfly-environment correlations**:
   - Previous day butterfly counts show weak correlations with most environmental variables
   - sum_butterflies_direct_sun shows moderate positive correlation with temperature metrics
...
**Comparison between correlation matrices:**

- **Full candidate set**: Many correlations >0.75, indicating substantial redundancy
- **Final model set**: Maximum correlation is ~0.78 (temp_max with hours_above_15C pathway), generally <0.5
- Variable selection successfully reduced multicollinearity while retaining diverse environmental dimensions

## 5. Modeling Approach

### 5.1 Two Model Families: Absolute vs. Proportional Effects

We tested **105 models** in two distinct families to distinguish different mechanisms by which environment affects abundance:
...
### 7.4 Interpretation of Effects

**Previous day baseline (butterflies_95th_percentile_t_1):**
- [Linear or smooth? Positive/negative? Interpretation]

**Temperature (temp_at_max_count_t_1):**
- [Describe shape of smooth term]
- [Biological interpretation]

**Wind (wind_max_gust_t_1):**
- [Describe shape of smooth term]
- **Note on unexpected pattern**: As described in project notes, wind effects show a non-intuitive pattern with largest decreases at zero wind and increasing/decreasing patterns at higher speeds
- Potential data artifact: Only one day with previous day wind = 0, which may drive unusual curve shape
- Warrants cautious interpretation

**Sun exposure (sum_butterflies_direct_sun_t_1):**
- [If included in best model, describe effect]
...
### 9.1 Evidence for Environmental Effects

**Baseline abundance is paramount:**
- All well-supported models include previous day butterfly count
- Environmental effects are proportional to population size, not absolute
- This suggests environmental factors modulate existing population trends rather than driving them

**Temperature effects:**
- [Summary based on actual results]

**Wind effects:**
- Present in top models but show complex, potentially non-intuitive patterns
- Effect magnitude and significance vary with model specification (p ≈ 0.08 in some versions)
- Curve shape may be influenced by data artifacts (single day with zero wind)
- More data might flatten or stabilize these patterns
...
This daily-level analysis **does not contradict** the 30-minute interval analysis:

- **30-minute analysis**: No detectable wind effect on instantaneous counts
- **Daily analysis**: Some evidence for wind effect on day-to-day persistence, but:
  - Effect is model-dependent and marginally significant
  - Relationship is complex and not easily interpretable
  - May represent population-level behavioral decisions (emigration) vs. instantaneous movement

### 9.3 Limitations and Caveats

**Sample size:**
- Only ~100 day-pairs vs. thousands of observations in 30-minute analysis
...
- Cannot follow individual butterflies; only observe population counts
- Longer time horizons would introduce ambiguity about population identity

**Wind effect interpretation:**
- Unexpected curve shape raises questions about biological plausibility
- Single outlier point (zero wind day) may drive unusual pattern
- Whether pattern is real or artifact requiring replication unclear
...
- Additional random effects or correlation structures might improve fit

### 9.4 Biological Interpretation

If wind effects are real (acknowledging uncertainty):

- Largest decreases at zero wind is counterintuitive
- Possible explanations:
  - Confounding with unmeasured variables (e.g., temperature inversions, microclimatic conditions)
  - Non-linear interaction between wind and temperature not captured by model
  - Artifact of limited sample size and data range

Field observations (Jessica Griffiths, personal observations) clearly show monarchs moving in response to wind. However:
- These are short-term, within-site movements
- May not translate to day-to-day emigration captured by peak counts
- Different temporal and spatial scales between field observations and this analysis

### 9.5 Recommendations for Future Work

1. **Increase sample size**: Additional years of data would:
   - Stabilize partial effect estimates
   - Improve power to detect effects
   - Allow assessment of year-to-year consistency

2. **Investigate zero-wind day**: Detailed examination of the single day with previous wind = 0
   - Is this a data quality issue?
   - What were other conditions on that day?
   - Does excluding it change inference?

3. **Alternative time windows**: Test 48-hour, 72-hour lags to understand temporal dynamics
   - Do effects accumulate over multiple days?
   - Is 24 hours the right response window?

4. **Individual tracking**: If feasible, mark-recapture or GPS tracking would:
   - Distinguish movement from mortality/emigration
   - Directly test within-site relocation vs. leaving
   - Connect field observations to population-level patterns

5. **Microclimate measurements**: Finer-scale environmental data at cluster locations
   - Temperature and wind measurements at cluster height
   - Sun exposure and shade mapping
   - Within-tree spatial heterogeneity

## 10. Conclusions

This daily-level analysis provides a complementary perspective to the 30-minute interval analysis:
...
2. **Complex environmental relationships**: Non-linear smooth terms outperform linear effects, indicating that environmental responses are not simple or monotonic

3. **Wind effects remain ambiguous**: Some evidence for wind effects at daily timescales, but:
   - Effect size and significance are model-dependent
   - Curve shapes are difficult to interpret biologically
   - Potential data artifacts warrant caution

4. **Complementary to prior analysis**: Results do not contradict the 30-minute interval findings
   - Different temporal scales may capture different processes
   - Instantaneous movement vs. emigration decisions

5. **Sample size matters**: Limited data (~100 observations) constrains inference strength
   - Multiple competitive models indicate uncertainty
   - Additional years of data would strengthen conclusions

This analysis demonstrates that environmental effects on monarch abundance are complex and scale-dependent. While it provides some support for wind effects over daily timescales, the mechanisms remain unclear and warrant further investigation with additional data and refined methods.
