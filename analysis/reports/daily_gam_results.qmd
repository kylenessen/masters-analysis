...
**Figure.** Normality assessment for candidate response variables and transformations. Each panel shows the distribution (histogram) with a normal curve overlay (red line). Rank indicates overall normality score, with Rank 1 being most normal. The normality score combines skewness, kurtosis, and formal normality tests.
...
### Butterfly Abundance Metrics (Previous Day)
...
### Sun Exposure Metrics (Previous Day)
...
### Temperature Metrics (Previous Day)
...
### Two Model Families: Absolute vs. Proportional Effects
...
### Models with Strong Support (ΔAIC < 2)
...
## Top 3 Models - Detailed Comparison
...
```{r load-top-3-summary}
# Load top 3 models detailed summary
top_3_summary <- read_csv(
    here("analysis", "reports", "figures", "top_3_models_summary.csv"),
    show_col_types = FALSE
)

kable(top_3_summary,
    digits = c(0, 0, 0, 0, 3, 3, 4, 4, 2),
    caption = "Top 3 Models: Summary Statistics"
) %>%
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```
...
- ΔAIC ranges from 0 to `r round(max(top_3_summary$Delta_AIC), 2)`, indicating strong empirical support for all three models
- Combined AIC weight: `r round(sum(top_3_summary$AIC_Weight), 3)` (models collectively account for most of the model weight)
- R² values are very similar: `r paste(round(top_3_summary$R_squared, 4), collapse = ", ")`
- All three models explain similar amounts of deviance

### Model 1 (Best Model)

**Formula:** `r top_3_summary$Formula[1]`

#### Model Output

```{r model-1-summary, comment=""}
# Read and display model summary text
model_1_summary <- readLines(
    here("analysis", "reports", "figures", "top_3_model_1_summary.txt")
)
cat(model_1_summary, sep = "\n")
```

#### Partial Effects

```{r model-1-partial, fig.width=14, fig.height=8}
knitr::include_graphics(
    here("analysis", "reports", "figures", "top_3_model_1_partial_effects.png")
)
```

**Figure.** Partial response curves for Model 1. Each panel shows the smooth effect of a predictor on the response variable (change in 95th percentile butterfly count), holding other variables constant.

#### Diagnostics

```{r model-1-diagnostics, fig.width=12, fig.height=10}
knitr::include_graphics(
    here("analysis", "reports", "figures", "top_3_model_1_diagnostics.png")
)
```

**Figure.** Diagnostic plots for Model 1 showing residual patterns, normality assessment, variance homogeneity, and residual distribution.

### Model 2

**Formula:** `r top_3_summary$Formula[2]`

**ΔAIC:** `r round(top_3_summary$Delta_AIC[2], 3)`

#### Model Output

```{r model-2-summary, comment=""}
# Read and display model summary text
model_2_summary <- readLines(
    here("analysis", "reports", "figures", "top_3_model_2_summary.txt")
)
cat(model_2_summary, sep = "\n")
```

#### Partial Effects

```{r model-2-partial, fig.width=14, fig.height=8}
knitr::include_graphics(
    here("analysis", "reports", "figures", "top_3_model_2_partial_effects.png")
)
```

**Figure.** Partial response curves for Model 2.

#### Diagnostics

```{r model-2-diagnostics, fig.width=12, fig.height=10}
knitr::include_graphics(
    here("analysis", "reports", "figures", "top_3_model_2_diagnostics.png")
)
```

**Figure.** Diagnostic plots for Model 2.

### Model 3

**Formula:** `r top_3_summary$Formula[3]`

**ΔAIC:** `r round(top_3_summary$Delta_AIC[3], 3)`

#### Model Output

```{r model-3-summary, comment=""}
# Read and display model summary text
model_3_summary <- readLines(
    here("analysis", "reports", "figures", "top_3_model_3_summary.txt")
)
cat(model_3_summary, sep = "\n")
```

#### Partial Effects

```{r model-3-partial, fig.width=14, fig.height=8}
knitr::include_graphics(
    here("analysis", "reports", "figures", "top_3_model_3_partial_effects.png")
)
```

**Figure.** Partial response curves for Model 3.

#### Diagnostics

```{r model-3-diagnostics, fig.width=12, fig.height=10}
knitr::include_graphics(
    here("analysis", "reports", "figures", "top_3_model_3_diagnostics.png")
)
```

**Figure.** Diagnostic plots for Model 3.

## Discussion
The wind effects show unexpected patterns in these results. The direct sunlight relationship is also interesting, with an initial dip followed by increases in cluster size with more light exposure. I'm uncertain about the interpretation of these results and whether they should be included in our conclusions. I'd appreciate your thoughts on these patterns. 
