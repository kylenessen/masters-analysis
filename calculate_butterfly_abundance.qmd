...
```{r}
#| label: patch-complete-days
#| echo: true

# Manually patch complete_day for specific day_ids that were incorrectly classified
days_to_patch <- c("SC1-20231120", "SC10-20240106", "SC10-20240131", "SC9-20240129")

cat("Patching complete_day values for:", paste(days_to_patch, collapse = ", "), "\n")

# Update complete_day to TRUE for these specific day_ids
abundance_index_filtered <- abundance_index_filtered %>%
    mutate(
...
# Also update the unique_days_per_deployment dataframe
unique_days_per_deployment <- unique_days_per_deployment %>%
    mutate(
        complete_day = case_when(
            day_id %in% days_to_patch ~ TRUE,
            TRUE ~ complete_day
...
cat("Patch applied successfully.\n")
```
...
#| fig-cap: "Animated GIF of all complete day timeseries plots"
#| echo: true
#| eval: false

# Load magick library for creating animations
library(magick)

# Create animated GIF of all complete day plots
cat("Creating animated GIF of all complete day plots...\n")

# Get all unique day_ids with complete_day = TRUE
all_day_ids <- abundance_index_filtered %>%
    filter(complete_day == TRUE) %>%
    pull(day_id) %>%
    unique() %>%
...
cat("Total plots to animate:", length(all_day_ids), "\n")

# Create temporary directory for individual plot files
temp_dir <- tempdir()
plot_files <- character(length(all_day_ids))

# Generate each plot and save as PNG
for (i in seq_along(all_day_ids)) {
    did <- all_day_ids[i]
    df_day <- abundance_index_filtered %>%
        filter(day_id == did)
    ...
    filename <- file.path(temp_dir, sprintf("plot_%03d.png", i))
    ggsave(filename, p, width = 10, height = 6, dpi = 150, bg = "white")
    plot_files[i] <- filename

    if (i %% 10 == 0) {
        cat("Processed", i, "of", length(all_day_ids), "plots\n")
    }
}

cat("All plots generated. Creating GIF...\n")

# Read all images
img_list <- image_read(plot_files)

# Create animated gif with 0.5 second delay (50 centiseconds)
animation <- image_animate(img_list, fps = 2, dispose = "previous")

# Save the animation
output_file <- "butterfly_plots_animation.gif"
image_write(animation, path = output_file)

cat("Animation saved as:", output_file, "\n")
cat("Frame rate: 2 fps (0.5 seconds per frame)\n")
cat("Total frames:", length(all_day_ids), "\n")
cat("Estimated duration:", length(all_day_ids) * 0.5, "seconds\n")
...
```{r}
#| label: export-complete-days
#| echo: true

# Export complete days only to CSV
complete_days_export <- abundance_index_filtered %>%
    filter(complete_day == TRUE) %>%
    select(-timestamp_str) %>%  # Drop timestamp_str column
    arrange(deployment_id, timestamp)

# Show summary of what we're exporting
cat("Exporting complete days dataset:\n")
cat("- Total rows:", nrow(complete_days_export), "\n")
cat("- Unique deployments:", n_distinct(complete_days_export$deployment_id), "\n")
cat("- Unique days:", n_distinct(complete_days_export$day_id), "\n")
cat("- Date range:", format(min(complete_days_export$timestamp), "%Y-%m-%d"),
    "to", format(max(complete_days_export$timestamp), "%Y-%m-%d"), "\n\n")

# Save to CSV
output_filename <- "data/butterfly_abundance_complete_days.csv"
readr::write_csv(complete_days_export, output_filename)
cat("Complete days dataset saved to:", output_filename, "\n")

# Display first few rows
cat("\nFirst 10 rows of exported data:\n")
print(head(complete_days_export, 10))
```
