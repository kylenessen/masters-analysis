...
Fixed version: properly handles missing isNight labels and filters unreasonable sunset times
"""
...
def is_reasonable_daylight_hour(timestamp):
    """Check if timestamp is within reasonable daylight hours (6 AM to 8 PM)"""
    hour = timestamp.hour
...
def load_all_sunset_times(json_dir: str = 'data/deployments'):
    ...
    for json_file in json_path.glob('*.json'):
        ...
        for image_filename, image_data in classifications.items():
            ...
            # Skip if not within reasonable daylight hours
            if not is_reasonable_daylight_hour(timestamp):
                continue

            date_key = timestamp.date()

            # Check isNight - treat missing as False only if within reasonable hours
        ...
        # Find sunset for each date (last non-night observation within reasonable hours)
...
def calculate_hours_until_sunset():
    ...
    print("Loading sunset times from all deployments (filtering to reasonable daylight hours)...")
    ...
    for idx, row in unique_days.iterrows():
        ...
        if sunset_time:
            ...
            # Only include if positive and reasonable (less than 12 hours)
            if 0 <= hours <= 12:
    ...
    if problematic_days:
        print(f"\nWarning: {len(problematic_days)} observations excluded due to unreasonable hours (>12):")
        for day in problematic_days[:5]:  # Show first 5
            print(f"  {day['deployment_id']} on {day['date'].date()}: {day['hours_calculated']:.1f} hours")
...
def create_histogram(df):
    ...
    n, bins, patches = ax.hist(df['hours_until_sunset'],
                               bins=np.arange(0, min(df['hours_until_sunset'].max() + 0.5, 12.5), 0.5),
                               edgecolor='black',
                               alpha=0.7,
                               color='darkblue')
    ...
    # Save figure
    output_path = 'analysis/dynamic_window_analysis/max_count_timing_until_sunset_histogram_fixed.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\nHistogram saved to: {output_path}")

    # Also save as HTML for interactive viewing
    html_path = 'analysis/dynamic_window_analysis/max_count_timing_until_sunset_analysis_fixed.html'
...
def create_html_report(df, output_path):
    ...
    <html>
    <head>
        <title>Maximum Butterfly Count Timing Analysis - Hours Until Sunset (Fixed)</title>
        <style>
            ...
            .warning {{ background-color: #f8d7da; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #dc3545; }}
    ...
    <body>
        ...
        <div class="note">
            ...
            of each day within reasonable daylight hours (6 AM - 8 PM).
        </div>

        <div class="warning">
            <strong>Data Cleaning Applied:</strong> Observations outside 6 AM - 8 PM are excluded from sunset determination.
            Days where the calculated time difference exceeds 12 hours are excluded as likely data errors.
        </div>
        ...
        <div class="plot">
            <img src="max_count_timing_until_sunset_histogram_fixed.png" alt="Histogram" style="max-width: 100%; height: auto;">
        </div>
    ...
    max_hours = min(df['hours_until_sunset'].max(), 12)
    bins = np.arange(0, max_hours + 1, 1)
    ...
    html_content += """
        ...
        <ul>
            ...
            <li>Sunset times are determined from the last non-night observation between 6 AM and 8 PM</li>
        </ul>
...
def main():
    ...
    print("="*60)
    print("MAXIMUM BUTTERFLY COUNT TIMING ANALYSIS (FIXED)")
    print("Analyzing when peak counts occur relative to SUNSET")
    print("Using filtered dataset (metrics_complete >= 0.95)")
    print("Sunset defined as last non-night observation between 6 AM - 8 PM")
    print("="*60)
    ...
    output_csv = 'analysis/dynamic_window_analysis/max_count_timing_until_sunset_analysis_fixed.csv'
