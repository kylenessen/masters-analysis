...
df <- read_csv(here("data", "lag_30.csv")) 

# Calculate change variables (t - t_lag)
df <- df %>%
  mutate(
    total_butterflies_change = total_butterflies_t - total_butterflies_t_lag,
    butterflies_direct_sun_change = butterflies_direct_sun_t - butterflies_direct_sun_t_lag,
    # Transformations to squish the distribution
    total_butterflies_change_cubert = sign(total_butterflies_change) * abs(total_butterflies_change)^(1/3),
    total_butterflies_change_sin = sin(total_butterflies_change / 50) # Scale before sin transformation
  )
...
numeric_vars <- df %>% 
  ungroup() %>%
  select(total_butterflies_t, butterflies_direct_sun_t, total_butterflies_change, 
         butterflies_direct_sun_change, total_butterflies_change_cubert, 
         total_butterflies_change_sin, temperature_avg,
...
p_compare2 <- ggplot(df) +
  geom_histogram(aes(x = total_butterflies_change), bins = 30, alpha = 0.5, fill = "green") +
  geom_histogram(aes(x = total_butterflies_change_sin * 100), bins = 30, alpha = 0.5, fill = "navy") +
  labs(title = "Original vs Sin Transform (scaled x100)", x = "Change Value", y = "Frequency")

grid.arrange(p_cube, p_sin, p_compare1, p_compare2, ncol = 2)
```
...
p15 <- ggplot(df, aes(x = temperature_avg, y = total_butterflies_change_sin)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "loess", se = TRUE, color = "navy") +
  labs(title = "Temperature vs Sin Change", x = "Average Temperature (°C)", y = "Sin of Change")

p16 <- ggplot(df, aes(x = avg_sustained, y = total_butterflies_change_sin)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "loess", se = TRUE, color = "navy") +
  labs(title = "Wind vs Sin Change", x = "Avg Sustained Wind (m/s)", y = "Sin of Change")
